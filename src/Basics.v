...
Fixpoint bin_to_nat (n : bin) : nat :=
  match n with
  | Ob => O
  | Tb n' => bin_to_nat n' + bin_to_nat n'
  | Sb n' => S (bin_to_nat n' + bin_to_nat n')
  end.

Example test_bin_incr1: bin_to_nat (incr Ob) = S (bin_to_nat Ob).
Proof. reflexivity. Qed.
Example test_bin_incr2: bin_to_nat (incr (Sb Ob)) = S (bin_to_nat (Sb Ob)).
Proof. reflexivity. Qed.
Example test_bin_incr3: bin_to_nat (incr (Tb Ob)) = S (bin_to_nat (Tb Ob)).
Proof. reflexivity. Qed.
Example test_bin_incr4: bin_to_nat (incr (Tb (Sb Ob))) = S (bin_to_nat (Tb (Sb Ob))).
Proof. reflexivity. Qed.
Example test_bin_incr5: bin_to_nat (incr (Sb (Tb Ob))) = S (bin_to_nat (Sb (Tb Ob))).
Proof. reflexivity. Qed.
...
Fixpoint foo (n : nat) : nat :=
  match n with
  | O => S O (* Change "S O" to "foo (S O)" to get an error. *)
  | S n' => O
  end.
(** [] *)
