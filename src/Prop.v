...
(* Infinite. Because we can use [b_0] any times. *)
(** [] *)
...
(*  No. Because [ev n' -> even n'] can not show [ev (S n') -> even (S n')]. *)
(** [] *)
...
(* Because [ev n'] has no power to proof [ev (S n')]. *)
*)
(** [] *)
...
(* [ev n'] has no power to proof [ev (S n')] thus we cannot complete the proof. *)
(** [] *)
...
Inductive pal {X : Type} : list X -> Prop :=
  ...
  | pal_app : forall x l, pal l -> pal (x :: (snoc l x)).
...
Lemma length_snoc : forall X : Type, forall n : X, forall l : list X,
  length (snoc l n) = S (length l).
Proof.
  intros X n l. induction l as [| n' l'].
  Case "l = nil".
    reflexivity.
  Case "l = cons n' l'".
    simpl. rewrite -> IHl'. reflexivity.  Qed.

Lemma rev_length : forall (X : Type) (l : list X),
...
Lemma ble_nat_length : forall (X : Type) (x : X) (l1 l2 : list X),
  x :: l1 = l2 -> S (length l1) = length l2.
Proof.
  intros X x l1 l2 H.
  rewrite <- H.
  reflexivity.
...
Lemma ble_nat_trans : forall (n m : nat),
...
Theorem pal_converse : forall (X : Type) (n : nat) (l : list X),
  ...
  Case "n = S n'".
    ...
    SCase "l = x :: l'".
      ...
      SSCase "l' = snoc l'' x'".
        ...
        rewrite -> H3 in H1.
        apply ble_nat_trans.
        rewrite -> ble_nat_length with (x:=x') (l2:=rev l').
        rewrite -> H3.
        apply H1.
        symmetry.
        apply H3.
        rewrite H4 in H3.
        rewrite -> rev_snoc in H3.
        inversion H3.
        rewrite -> rev_involutive.
        symmetry.
        apply H5.
...
Proof.
  intros X l H.
  apply pal_converse with (n:=length l).
  symmetry.
  apply ble_nat_refl.
  apply H.
Qed.
...
(** - Which of the following propositions are provable?
    ...
    [R a b c] = [a + b = c].
    Thus [R 1 1 2] holds but [R 2 2 6] doesn't hold.

    Drop [c4] or [c5] doesn't change anything because
    [c1 c2 c3] still holds for [R a b c] = [a + b = c].
[]
*)
...
Proof.
  intros l1 l2 l3 S1 S2.
  generalize dependent l1.
  induction S2 as [| n l| n l].
  Case "sub_nil".
    intros l1 S1.
    apply S1.
  Case "sub_addr".
...
(** Suppose we give Coq the following definition:
    ...
    R 0 [] -> R 1 [0] -> R 2 [1 0] -> R 3 [2 1 0] -> R 2 [2 1 0] -> 
    R 1 [2 1 0] -> R 2 [1 2 1 0] -> R 1 [1 2 1 0]

    R 6 [3 2 1 0] is impossible.
*)  
