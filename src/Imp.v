...
Proof.
  ...
  destruct n;
      try (simpl; rewrite IHa1; try (rewrite plus_0_r); reflexivity).
Qed.
(** [] *)
...
Proof.
  ...
  Case "->".
    intros H; induction H; subst;
      ...
      try (rewrite <- H; rewrite <- H0);
      reflexivity.
  Case "<-".
    generalize dependent b.
    induction e; simpl; intros; subst; constructor;
       try apply aeval_iff_aevalR; try apply IHe;
       try apply IHe1; try apply IHe2; try reflexivity.
Qed.
...
Inductive no_whilesR: com -> Prop :=
  ...
  | N_If : forall e c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (IFB e THEN c1 ELSE c2 FI)
  .
...
Proof.
  ...
  Case "SKIP".
    ...
    constructor.
  Case "::=".
    exists (update st x (aeval st e)).
    constructor.
    reflexivity.
  Case ";;".
    assert (exists st' : state, c1 / st || st') by apply IHno_whilesR1.
    inversion H1.
    assert (exists st' : state, c2 / x || st') by apply IHno_whilesR2.
    inversion H3.
    exists x0.
    apply E_Seq with x; assumption.
  Case "IF".
    destruct (beval st e) eqn:H1.
    SCase "THEN".
      assert (exists st' : state, c1 / st || st') by apply IHno_whilesR1.
      inversion H2.
      exists x.
    ...
    SCase "ELSE".
      assert (exists st' : state, c2 / st || st') by apply IHno_whilesR2.
      inversion H2.
      exists x.
...
Definition s_pop (stack : list nat) (f : nat -> nat -> nat) : list nat :=
  match stack with
  | x :: y :: stack' => f y x :: stack'
  | _ => []
  end.

Definition s_execute_one (st : state) (stack : list nat) (prog : sinstr) : list nat :=
  match prog with
  | SPush n => n :: stack
  | SLoad id => (st id) :: stack
  | SPlus => s_pop stack plus
  | SMinus => s_pop stack minus
  | SMult => s_pop stack mult
  end.
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  match prog with
  | nil => stack
  | s :: t => s_execute st (s_execute_one st stack s) t
  end.
...
Lemma s_compile_general : forall (e : aexp) (st : state) (l : list nat) (prog : list sinstr),
  s_execute st l (s_compile e ++ prog) = s_execute st ((aeval st e) :: l) prog.
Proof.
  intros e.
  aexp_cases (induction e) Case; intros; try reflexivity;
    simpl; repeat rewrite -> app_ass; rewrite -> IHe1; rewrite -> IHe2; reflexivity.
Qed.
...
Proof.
  intros.
  assert ([aeval st e] = s_execute st [aeval st e] nil) by reflexivity.
  rewrite H.
  assert (s_compile e = s_compile e ++ nil).
    rewrite <- app_nil_end.
    reflexivity.
  rewrite H0.
  apply s_compile_general.
Qed.
(** [] *)
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_SeqBreak : forall c1 c2 st st', c1 / st || SBreak / st' ->
      (c1 ;; c2) / st || SBreak / st'
  | E_SeqContinue : forall c1 c2 st st' st'' sta, c1 / st || SContinue / st' -> 
      c2 / st' || sta / st'' -> (c1 ;; c2) / st || sta / st''
  | E_IfTrue : forall st st' b c1 c2 sta, beval st b = true -> c1 / st || sta / st' ->
      (IFB b THEN c1 ELSE c2 FI) / st || sta / st'
  | E_IfFalse : forall st st' b c1 c2 sta, beval st b = false -> c2 / st || sta / st' ->
      (IFB b THEN c1 ELSE c2 FI) / st || sta / st'
  | E_WhileEnd : forall b st c, beval st b = false ->
      (WHILE b DO c END) / st || SContinue / st
  | E_WhileLoopBreak : forall st st' b c, beval st b = true -> c / st || SBreak / st' ->
      (WHILE b DO c END) / st || SContinue / st'
  | E_WhileLoopContinue : forall st st' st'' b c, beval st b = true -> c / st || SContinue / st' ->
      (WHILE b DO c END) / st' || SContinue / st'' -> (WHILE b DO c END) / st || SContinue / st''

  where "c1 '/' st '||' s '/' st'" := (ceval c1 st s st').

Tactic Notation "ceval_cases" tactic(first) ident(c) :=
  first;
  [ Case_aux c "E_Skip" | Case_aux c "E_Break" | Case_aux c "E_Ass" | Case_aux c "E_SeqBreak" 
  | Case_aux c "E_Continue" | Case_aux c "E_IfTrue" | Case_aux c "E_IfFalse" 
  | Case_aux c "E_WhileEnd" | Case_aux c "E_WhileLoopBreak" | Case_aux c "E_WhileLoopContinue" 
  ].
...
Proof.
  intros.
  remember (WHILE b DO c END) as w eqn: H1.
...
Proof.
  ...
  generalize dependent st2.
  ceval_cases (induction H) Case; intros st2 s2 HH; inversion HH; subst;
...
Fixpoint beval_sc (st : state) (b : bexp) : bool :=
  ...
  | BAnd b1 b2  => match beval_sc st b1 with
                   | false => false
                   | true => beval_sc st b2
                   end
  end.

Theorem short_circuit : forall st b, beval st b = beval_sc st b.
Proof.
  intros.
  bexp_cases (induction b) Case; reflexivity.
Qed.
(** [] *)
