...
Proof.
  ...
  Case "tapp".
    inversion Ht; subst.
    right.
    assert (\empty |- t1 \in TArrow T11 T) by apply H2.
    apply IHt1 in H2.
    apply IHt2 in H4.
    destruct H2.
    destruct H4.
    assert (exists x u, t1 = tabs x T11 u).
      eapply canonical_forms_fun.
      apply H.
      apply H0.
  ...
  Case "tif".
    inversion Ht; subst.
    right.
    assert (\empty |- t1 \in TBool) by apply H3.
    apply IHt1 in H3.
    destruct H3.
...
(** An exercise in the [Types] chapter asked about the subject
    ...
    No, [tif ttrue ttrue (tvar x)] is a counter example.
[]
*)

Example subject_expansion_stlc_false : exists t t' T,
  t ==> t' /\ empty |- t' \in T /\ ~ empty |- t \in T.
Proof.
  exists (tif ttrue ttrue (tvar x)).
...
Theorem types_unique : forall t gamma T T',
  T <> T' -> gamma |- t \in T -> ~ gamma |- t \in T'.
Proof.
  intros t.
  t_cases (induction t) Case; intros gamma T T' H H0 H1;
    inversion H0; subst; clear H0;
    inversion H1; subst; clear H1.
  Case "tvar".
    rewrite H4 in H3.
...
(* progress : If some [tm] has type, it's a value or it can make a step *)
(* preservation : If some [tm] has type and make a step, the stepped [tm] also has that type *)
...
and the following typing rule:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        becomes false
        [tif ttrue ttrue tfalse] ==> [ttrue] by [ST_IfTrue]
        [tif ttrue ttrue tfalse] ==> [zap]   by [ST_Zap]
...
(** Suppose instead that we add a new term [foo] with the following reduction rules:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        becomse false
        [(\x:Bool .x) ttrue] ==> [ttrue]    by [ST_AppAbs]
        [(\x:Bool .x) ttrue] ==> [foo true] by [ST_Foo1]
...
(** Suppose instead that we add the following new rule to the reduction relation:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        becomes false
        [tif ttrue tfalse tfalse] ==> [tfalse] by [ST_IfTrue]
        [tif ttrue tfalse tfalse] ==> [ttrue]  by [ST_FunnyIfTrue]
...
Tactic Notation "step_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "ST_SuccNat" | Case_aux c "ST_PredNat"
  | Case_aux c "ST_MultNat" ].
...
Proof with eauto.
  ...
  Case "ST_App2".
    inversion H3; subst; inversion Hy1.
...
Tactic Notation "has_type_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "T_If0" | Case_aux c "T_Succ"
...
Proof.
  ...
  Case "tsucc".
    inversion Ht; subst.
    right.
    assert (\empty |- t \in TNat) by apply H1.
    apply IHt in H1.
    destruct H1.
    assert (exists n, t = tnat n).
      apply canonical_forms_nat.
      apply H.
      apply H0.
    inversion H1; subst; clear H1.
    eauto.
  ...
  Case "tmult".
    inversion Ht; subst.
    right.
    assert (\empty |- t1 \in TNat) by apply H2.
    apply IHt1 in H2.
    destruct H2.
    assert (\empty |- t2 \in TNat) by apply H4.
    apply IHt2 in H4.
    destruct H4.
  ...
  Case "tif0".
    inversion Ht; subst.
    right.
    assert (\empty |- t1 \in TNat) by apply H3.
    apply IHt1 in H3.
    destruct H3.
    assert (exists n, t1 = tnat n).
      apply canonical_forms_nat.
...
Tactic Notation "afi_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "afi_if01" | Case_aux c "afi_if02" 
  | Case_aux c "afi_if03" | Case_aux c "afi_succ"
  | Case_aux c "afi_pred" | Case_aux c "afi_mult1"
