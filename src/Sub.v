...
(** Suppose we had incorrectly defined subtyping as covariant on both
    ...
    execution.
       ...
       [f] = [\r:Student r.gpa]
       [g] = [\r:(Person -> nat) r {name='Pat',age=21}]
...
Where does the type [Top->Top->Student] fit into this order?
  ...
  [Top -> Top -> Student] <: [Student -> Top] <: [Top]
...
(** Is the following statement true or false?  Briefly explain your
    ...
    False, [T] = [A * A] and [A] has no subtype.
]] 
[] *)
...
(** How many supertypes does the record type [{x:A, y:C->C}] have?  That is,
    ...
    {x:A} {x:Top} {y:C->C} {y:C->Top} {y:Top} {} Top
    Total : 19
...
Not a good idea. The functions have type [T1*T2 -> S] will get stuck with input has type [T2*T1]
...
Definition Person : ty := 
  TProd String (TProd TTop TTop).
Definition Student : ty := 
  TProd String (TProd Float TTop).
Definition Employee : ty := 
  TProd String (TProd TTop Integer).
...
Proof.
  apply T_Pair; apply T_Abs; apply T_Var; apply eq_id.
Qed.
(** [] *)
...
Proof with auto.
  intros U Hs.
  remember TBool as V.
  subtype_cases (induction Hs) Case; subst; try solve by inversion...
  Case "S_Trans".
    eapply eq_trans.
    apply IHHs1...
    apply IHHs2...
...
Proof with eauto.
  ...
  subtype_cases (induction Hs) Case; 
    intros; subst; try solve by inversion...
  Case "S_Trans".
    destruct (IHHs2 V1 V2)...
    destruct H.
    destruct H.
    destruct H0.
    subst.
...
Proof with eauto.
  ...
  has_type_cases (induction Hty) Case; intros; try solve by inversion...
  Case "T_Sub".
    subst. 
...
Lemma typing_inversion_fst : forall Gamma t1 T,
  Gamma |- (tfst t1) \in T ->
  exists S, Gamma |- t1 \in (TProd T S).
Proof with eauto.
  intros Gamma t1 T Hty.
  remember (tfst t1) as t.
  has_type_cases (induction Hty) Case; intros;
    inversion Heqt; subst; try solve by inversion...
  Case "T_Sub".
    apply IHHty in H0.
...
Lemma typing_inversion_snd : forall Gamma t1 T,
  Gamma |- (tsnd t1) \in T ->
  exists S, Gamma |- t1 \in (TProd S T).
Proof with eauto.
  intros Gamma t1 T Hty.
  remember (tsnd t1) as t.
  has_type_cases (induction Hty) Case; intros;
    inversion Heqt; subst; try solve by inversion...
  Case "T_Sub".
    apply IHHty in H0.
...
Proof with eauto.
  intros. 
  remember (TProd S1 S2) as S.
  remember (TProd T1 T2) as T.
  generalize dependent T1.
  generalize dependent T2.
  subtype_cases (induction H) Case; intros; subst; try solve by inversion.
  Case "S_Refl".
    inversion HeqT...
  Case "S_Trans".
...
Proof with eauto.
  intros.
  edestruct typing_inversion_pair.
  apply H.
  destruct H0.
  destruct H0.
  destruct H1.
  apply prod_types in H0.
  destruct H0...
Qed.
...
Proof with eauto.
  ...
  Case "tpair".
    apply typing_inversion_pair in Htypt.
    destruct Htypt.
    destruct H.
    destruct H.
    destruct H0...
  Case "tfst".
    apply typing_inversion_fst in Htypt.
    destruct Htypt...
  Case "tsnd".
    apply typing_inversion_snd in Htypt.
    destruct Htypt...
Qed.
...
(** Each part of this problem suggests a different way of
    ...
    - Suppose we _change_ the arrow subtyping rule to:
                          ...
                          -----------------------                    (S_Arrow')
                               S1->S2 <: T1->T2
      preservation
        [(\f:Person->Nat. (f P)) (\x:Student. (x.gpa))] : [Nat]
        [(\x.Student (x.gpa)) P]                        : None
[] *) 
