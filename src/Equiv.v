...
Proof.
  ...
  Case "<-".
    inversion H; subst;
      [apply E_IfFalse | apply E_IfTrue];
      try (simpl in H5; apply negb_true_iff; apply H5);
      try (simpl in H5; apply negb_false_iff; apply H5);
      apply H6.
Qed.
(** [] *)
...
(** Write an informal proof of [WHILE_false].
  ...
  The guard [b] is [BFalse] means then [WHILE] won't executed.
  Which is the same as [SKIP].
[]
*)
...
(** Explain what the lemma [WHILE_true_nonterm] means in English.
  ...
  The guard [b] is [BTrue] means the [WHILE] won't terminated.
*)
(** [] *)
...
Proof.
  ...
  Case "->".
    ...
    reflexivity.
    inversion H0; subst.
    assert (aeval st' (AId X) = aeval st' e) by apply H.
    inversion H1; subst.
    rewrite H3.
    rewrite <- H1.
    simpl.
...
Proof. 
  ...
  split; intros; inversion H1; subst;
    ...
    try (apply H; apply H4);
    try (apply H0; apply H7).
Qed.
(** [] *)
...
Proof.
  ...
  Case "BLe". 
    simpl.
    remember (fold_constants_aexp a) as a' eqn:H.
    remember (fold_constants_aexp a0) as a0' eqn:H1.
...
Proof. 
  ...
  Case "WHILE".
    ...
    destruct (fold_constants_bexp b) eqn:H1;
...
Fixpoint optimize_0plus_bexp (e : bexp) : bexp :=
  ...
  | BNot e1 => BNot (optimize_0plus_bexp e1)
...
Proof.
  ...
  bexp_cases (induction b) Case; simpl;
    try (rewrite <- optimize_0plus_aexp_sound; rewrite <- optimize_0plus_aexp_sound);
    try rewrite <- IHb;
...
Example optimize_0plus_fold_constants_com_example :
  optimize_0plus_fold_constants_com 
    (X ::= APlus (AMinus (ANum 1) (ANum 1)) (AId Y)) =
    X ::= AId Y.
Proof. reflexivity.  Qed.

Theorem optimize_0plus_fold_constants_com_sound : 
...
Theorem better_subst_equiv : forall i1 i2 a1 a2, var_not_used_in_aexp i1 a1 ->
...
Proof.
  ...
  unfold not.
  intros.
  assert (SKIP / empty_state || empty_state) by apply E_Skip.
  apply H in H0.
  assert (~ (WHILE BTrue DO SKIP END) / empty_state || empty_state) by apply loop_never_stops.
  contradiction.
Qed.
(** [] *)
...
Proof.
  ...
  Case "->".
    ...
    inversion H0.
  Case "<-".
    replace (update (update st Y n) X n0) with (update (update st X n0) Y n).
...
Proof.
  ...
  unfold not.
  intros.
  assert (ptwice / empty_state || update (update empty_state X 0) Y 1).
  ...
  inversion H6; subst.
  simpl in H7.
  assert (update (update empty_state X 0) Y 1 Y = 1) by apply update_eq.
  assert (update st' Y (st' X) X = update st' Y (st' X) Y).
    reflexivity.
  rewrite H7 in H2.
  rewrite <- H2 in H1.
  assert (update (update empty_state X 0) Y 1 X = 0).
...
Proof.
  unfold not; intros.
  remember p1 as p.
  ceval_cases (induction H0) Case; simpl;
    try inversion Heqp;
    subst.
  Case "E_WhileEnd".
    apply H.
...
Proof.
  unfold cequiv; unfold p1; unfold p2; split; intros.
  Case "->".
    inversion H; subst.
    apply E_WhileEnd.
    apply H4.
    apply E_WhileLoop with st'0;
      try apply H2;
      simpl in H2; apply negb_true_iff in H2; apply beq_nat_false in H2;
      apply p1_may_diverge with (st:=st) (st':=st') in H2;
      contradiction.
  Case "<-".
    inversion H; subst.
    apply E_WhileEnd.
    ...
    apply E_WhileLoop with st'0;
      try apply H2;
      simpl in H2; apply negb_true_iff in H2; apply beq_nat_false in H2;
      apply p2_may_diverge with (st:=st) (st':=st') in H2;
      contradiction.
Qed.
(** [] *)
...
Proof.
  assert (p3 / (update empty_state X 1) || 
  ...
  inversion H6; subst.
  simpl in H7.
  assert (update st' Z 1 Z = 1) by reflexivity.
  assert (update (update (update (update empty_state X 1) Z 1) X 0) Z 0 Z = 0) by reflexivity.
  rewrite <- H7 in H2.
  rewrite H1 in H2.
  inversion H2.
Qed.
...
Proof.
  ...
  Case "->".
    assert (st' = update st X 1).
      ...
      remember (WHILE BNot (BEq (AId X) (ANum 1)) DO HAVOC X END) as p.
      ceval_cases (induction H) SCase; simpl; inversion Heqp; subst.
      SCase "E_WhileEnd".
        simpl in H.
  ...
  Case "<-".
    inversion H; subst.
    simpl.
    destruct (beq_nat (st X) 1) eqn:H1.
    SCase "X = 1".
...
Proof.
  ...
  bexp_cases (induction b) Case; simpl;
    try rewrite IHb;
    try (rewrite IHb1; rewrite IHb2);
    try (rewrite stequiv_aeval);
    try rewrite stequiv_aeval with (st1:=st1) (st2:=st2) (a:=a);
    try rewrite stequiv_aeval with (st1:=st1) (st2:=st2) (a:=a0);
    try (unfold stequiv; apply H);
    try reflexivity.
Qed.
(** [] *)
...
Proof.
    ...
    Case "<-".  
      ...
      inversion H0; subst.
      simpl in H1.
      apply stequiv_trans with (update st X (st X)).
      unfold stequiv.
      intros.
      rewrite update_same; reflexivity.
      apply H1.
...
*)
Definition cequiv'' (c1 c2 : com') : Prop :=
  forall st st' : state, Imp.ceval' c1 st st' <-> Imp.ceval' c2 st st'.

Theorem for_while_equiv : forall b c1 c2 c3,
  cequiv'' (CFor' c1 b c2 c3) (CSeq' c1 (CWhile' b (CSeq' c2 c3))).
Proof.
  unfold cequiv''.
  split; intros.
  Case "->".
...
(* Hint: You'll need [functional_extensionality] *)
  ...
  split; intros;
    ...
    [replace (update (update st l1 (aeval st a1)) l2 (aeval st a2)) with
      (update (update st l2 (aeval st a2)) l1 (aeval st a1))
    |replace (update (update st l2 (aeval st a2)) l1 (aeval st a1)) with
      (update (update st l1 (aeval st a1)) l2 (aeval st a2))];
    try (apply E_Ass; rewrite aeval_weakening);
    try reflexivity;
    try assumption;
    apply functional_extensionality; intros;
    apply update_permute; try assumption.
    unfold not.
    intros.
    symmetry in H3.
    contradiction.
