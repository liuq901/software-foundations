...
(** Paraphrase the following Hoare triples in English.
   1) {{True}} c {{X = 5}} 
      X will become 5 after c

   2) {{X = m}} c {{X = m + 5)}}
      X will be added by 5 after c

   3) {{X <= Y}} c {{Y <= X}}
      Y <= X after c if X <= Y before c
   ...
   5) {{X = m}} 
      c
      {{Y = real_fact m}}.
      Y will hold some real_fact of X after C

   6) {{True}} 
      c 
      {{(Z * Z) <= m /\ ~ (((S Z) * (S Z)) <= m)}}
      Z will become the square root of m
 *)
...
Proof.
  ...
  Case "Then".
    eapply hoare_consequence_pre.
    eapply hoare_asgn.
    unfold bassn, assn_sub, update, assert_implies; intros.
    simpl.
    inversion H; subst.
    simpl in H1.
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileLoop : forall (st st' st'' : state) (b1 : bexp) (c1 : com),
                  ...
                  (WHILE b1 DO c1 END) / st || st''
  | E_If1 : forall (st st' : state) (b : bexp) (c : com),
            (IFB b THEN c ELSE SKIP FI) / st || st' ->
...
Theorem hoare_if : forall P Q b c,
...
Proof.
  ...
  Case "ELSE".
    ...
    inversion H; subst.
    simpl in H1.
    destruct (beq_nat (st Y) 0) eqn:H2.
...
Inductive ceval : state -> com -> state -> Prop :=
  ...
  | E_WhileLoop : forall st st' st'' b1 c1,
      ...
      ceval st' (WHILE b1 DO c1 END) st'' ->
      ceval st (WHILE b1 DO c1 END) st''
  | E_Repeat : forall st st' b1 c1,
      ceval st (c1 ;; WHILE (BNot b1) DO c1 END) st' ->
...
Proof.
  unfold hoare_triple; subst.
  intros.
  inversion H1; subst.
  inversion H7; subst.
  apply H in H6.
  remember (WHILE BNot b DO c END) as p.
  ceval_cases (induction H9) Case; inversion Heqp; subst.
  Case "E_WhileEnd".
    split.
    assumption.
...
Definition havoc_pre (X : id) (Q : Assertion) : Assertion :=
  fun st => forall x, Q (update st X x).
