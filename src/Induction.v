...
(** Briefly explain the difference between the tactics
    ...
    [indution] has induction hypothesis but [destruct] not.
...
Fixpoint normalize (n : bin) : bin :=
  match n with
  | Ob => Ob
  | Sb n' => incr (double_bin (normalize n'))
  | Tb n' => double_bin (normalize n')
  end.

Lemma normalize_double_bin_comm : forall n : bin,
  double_bin (normalize n) = normalize (double_bin n).
Proof.
  intros n.
  destruct n.
  reflexivity.
...
Lemma normalize_incr_comm : forall n : bin,
...
Lemma normalize_fix : forall n : bin,
...
Proof.
  ...
  Case "n = Sb n'".
    ...
    rewrite <- normalize_double_bin_comm.
    rewrite -> IHn'.
    reflexivity.
  Case "n = Tb n'".
    simpl.
...
Lemma normalize_plus : forall n : nat,
  nat_to_bin (n + n) = normalize (Tb (nat_to_bin n)).
Proof.
  intros n.
  induction n as [| n'].
  Case "n = 0".
...
Theorem bin_to_mat_to_bin : forall n : bin,
...
Proof.
  ...
  Case "n = Sb n'".
    simpl.
    rewrite -> normalize_plus.
    rewrite -> normalize_incr_comm.
    simpl.
    rewrite -> IHn'.
    rewrite -> normalize_fix.
    reflexivity.
  Case "n = Tb n'".
    simpl.
    rewrite -> normalize_plus.
