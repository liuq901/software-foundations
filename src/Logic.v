...
(** Write an informal proof of [double_neg]:
   ...
   _Proof_:
   [~~P] is [(P -> False) -> False].
   Since [P] is [True] and [P -> False], [False] in [True].
   []
*)
...
Theorem excluded_middle_de_morgan_not_and_not : excluded_middle -> de_morgan_not_and_not.
Proof.
  unfold excluded_middle.
  unfold de_morgan_not_and_not.
  unfold not.
  intros H1 P Q H2.
  assert (P \/ (P -> False)) as [H3 | H4].
  apply H1.
  left.
  apply H3.
  assert (Q \/ (Q -> False)) as [H5 | H6].
  apply H1.
  right.
  apply H5.
