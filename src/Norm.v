...
Proof with eauto.   
   ...
   Case "ST_Pair2".
     ...
     apply IHHy1 in H0.
     rewrite H0...
     assert (value (tpair y2 v2))...
     apply value__normal in H.
     exfalso...
   Case "ST_FstPair".
     assert (value (tpair v1 v2))...
     apply value__normal in H1.
     exfalso...
   Case "ST_Snd".
     apply IHHy1 in H0.
     rewrite H0...
     assert (value (tpair v1 y2))...
     apply value__normal in H.
     exfalso...
   Case "ST_SndPair".
     assert (value (tpair v1 v2))...
...
Fixpoint R (T:ty) (t:tm) {struct T} : Prop :=
  ...
  (match T with
   ...
   | TProd T1 T2 => (exists s1 s2, R T1 s1 /\ R T2 s2 /\ t ==>* tpair s1 s2)
   end).
...
Lemma has_type_injective : forall t T1 T2,
  has_type \empty t T1 -> has_type \empty t T2 -> T1 = T2.
Proof with eauto.
  intros.
...
Proof.
  ...
  Case "TArrow".
    ...
    eapply IHT2; eauto.
    apply H3 in H5.
    apply R_typable_empty in H5.
    inversion H5; subst; clear H5.
    eapply preservation in H0; eauto.
    eapply T_App.
    eauto.
    assert (TArrow T0 T2 = TArrow T1 T2).
      eapply has_type_injective; eauto.
    inversion H5; subst; clear H5.
    apply H11.
  Case "TProd".
    inversion H1; subst; clear H1.
...
Proof with eauto.
  intros.
  t_cases (induction t) Case; simpl...
  Case "tvar".
...
Proof with eauto.
 t_cases (induction t) Case; intros; simpl.
  ...
  Case "tapp".
    assert ([x1 := v1]([x := v]t1) = [x := v]([x1 := v1]t1))...
    assert ([x1 := v1]([x := v]t2) = [x := v]([x1 := v1]t2))...
    rewrite H2.
    rewrite H3...
  Case "tabs".
    assert ([x1 := v1]([x := v]t0) = [x := v]([x1 := v1]t0))...
    destruct (eq_id_dec x1 i)...
    destruct (eq_id_dec x i)...
    rewrite H2...
  Case "tpair".
    assert ([x1 := v1]([x := v]t1) = [x := v]([x1 := v1]t1))...
    ...
    rewrite H3...
  Case "tfst".
    assert ([x1 := v1]([x := v]t) = [x := v]([x1 := v1]t))...
    rewrite H2...
  Case "tsnd".
    assert ([x1 := v1]([x := v]t) = [x := v]([x1 := v1]t))...
    rewrite H2...
  ...
  Case "tif".
    assert ([x1 := v1]([x := v]t1) = [x := v]([x1 := v1]t1))...
    assert ([x1 := v1]([x := v]t2) = [x := v]([x1 := v1]t2))...
    assert ([x1 := v1]([x := v]t3) = [x := v]([x1 := v1]t3))...
    rewrite H2.
    rewrite H3.
    rewrite H4...
Qed.
...
Proof. 
  ...
  Case "T_Pair".
    rewrite msubst_pair.
    assert (R T1 (msubst env0 t1)) by eauto.
    assert (R T2 (msubst env0 t2)) by eauto.
    destruct (R_halts H0) as [v1 [P1 Q1]].
    destruct (R_halts H1) as [v2 [P2 Q2]].
    assert (tpair (msubst env0 t1) (msubst env0 t2) ==>* tpair v1 v2).
      eapply multi_trans.
      apply multistep_Pair1; eauto.
      apply multistep_Pair2; eauto.
    repeat split; eauto.
    apply T_Pair; apply R_typable_empty; eauto.
    exists (tpair v1 v2); eauto.

  Case "T_Fst".
    rewrite msubst_fst.
    assert (R (TProd T1 T2) (msubst env0 t)) by eauto.
    destruct H0.
    destruct H1.
    destruct H2.
    destruct H2.
    destruct H2.
    destruct H3.
    destruct (R_halts H2) as [v1 [P1 Q1]].
    destruct (R_halts H3) as [v2 [P2 Q2]].
    assert (tfst (msubst env0 t) ==>* v1).
      eapply multi_trans.
      apply multistep_Fst; eauto.
      eapply multi_trans.
      apply multistep_Fst.
      apply multistep_Pair1; eauto.
      eapply multi_trans.
  ...
  Case "T_If".
    rewrite msubst_if.
    assert (R TBool (msubst env0 t0)) by eauto.
    assert (R T (msubst env0 t1)) by eauto.
    assert (R T (msubst env0 t2)) by eauto.
    destruct H0.
    destruct H3.
    destruct H3.
    destruct H3.
    assert (has_type empty (tif (msubst env0 t0) (msubst env0 t1) (msubst env0 t2)) T).
      apply T_If; try apply R_typable_empty; eauto.
    eapply multistep_preserves_R'.
    eauto.
    apply multistep_If; eauto.
    assert (has_type \empty x TBool).
