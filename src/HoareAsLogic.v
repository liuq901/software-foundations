...
Proof.
  unfold hoare_triple.
  intros P c Q X.
  hoare_proof_cases (induction X) Case; intros; inversion H; subst; eauto.
  Case "H_If".
    eapply IHX2.
    apply H7.
  ...
  Case "H_While".
    ...
    clear H3 H4.
    ceval_cases (induction H) SCase; inversion Heqp; subst.
    SCase "E_WhileEnd".
      split; try apply bexp_eval_false; assumption.
    SCase "E_WhileLoop".
...
Proof.
  ...
  Case "WHILE".
    ...
    assert ((WHILE b DO c END) / st || st).
      eapply E_WhileEnd; try apply bassn_eval_false; eauto.
    apply H0.
    apply H.
Qed.
(** [] *)
