...
(** _Proof_: By induction on a derivation of [|- t \in T].
     ...
     - If the last rule is [T_Succ], then [t = tsucc t1].
           
            - If [t1] is a value, then [t] is a value.

            - If [t1] can take a step, so can [t].
            
     - If the last rule is [T_Pred] or [T_Iszero], then [t = tpred t1] or
       [t = tiszero t1]. [t] can always make a step.
[] *)
...
(** _Proof_: By induction on a derivation of [|- t \in T].
    ...
    - If the last rule is [T_Succ] or [T_Iszero], then [t = tsucc t1] or 
      [t = tiszero t1]. [t] has type since [t1] has type.

    - If the last rule is [T_Pred], then [t = pred t1].

           - If [t1 = tzero] then [t = tzero] and [t] has type.
            
           - If [t1 = succ t1'] then [t = t1'] and since [t1'] has type, [t] has type.

           - If [t1 ==> t1'] and [t1'] has type then [t] has type.
[] *)
...
Proof with eauto.
  intros t t' T HT HE.
  generalize dependent T.
  step_cases (induction HE) Case; intros T HT; inversion HT; subst; clear HT...
  Case "ST_PredSucc".
    inversion H1...
Qed.
(** [] *)
...
(** Having seen the subject reduction property, it is reasonable to
    ...
    False, [tif ttrue ttrue tzero] is a counter-example.
[] *)

Example subject_expansion_false : exists t t' T,
  t ==> t' /\ |- t' \in T /\ ~ |- t \in T.
Proof.
...
(** Suppose, instead, that we add this new rule to the [step] relation: 
   ...
   this rule?  For each one that does, give a counter-example.
        ...
        [tif ttrue ttrue tfalse] ==> [tfalse] by [ST_Funny1]
[] *)
...
(** Suppose instead that we add this rule:
   ...
   this rule?  For each one that does, give a counter-example.
        ...
        Determinism of [step]
        [tif ttrue (tif tfalse ttrue tfalse) tfalse] ==>
        [tif tfalse ttrue tfalse] by [ST_IfTrue]
        [tif ttrue (tif tfalse ttrue tfalse) tfalse] ==>
        [tif ttrue tfalse tfalse] by [ST_Funny2]
[] *)
...
(** Make up some exercises of your own along the same lines as
    ...
    break just one of the properties and leave the others alone.
      ...
      - Progress
        Add rule to [has_type] which cannot [step]

      - Preservation
        Add rule to [has_type] which can [step]
[] *)
...
(** The evaluation rule [E_PredZero] is a bit counter-intuitive: we
    ...
    [step]?  Would doing so create any problems elsewhere? 
        ...
        Progress will become false due to [tpred tzero] will be a counter-example.
[] *)
...
(** Suppose our evaluation relation is defined in the big-step style.
    ...
    properties?
      ...
      - Progress
        If type is correct, finishes with a value

      - Preservation
        If type is correct, the final value has the same type
[] *)
