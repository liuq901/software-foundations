...
(*
  ...
  P bempty -> (forall y : yesno, P (bleaf y)) -> (forall (y : yesno) (b1 : byntree), 
  P b1 -> forall b2 : byntree, P b2 -> P (nbranch y b1 b2)) -> forall b : byntree, P b
*)
(** [] *)
...
(*
  tree_ind : forall (X : Type) (P : tree X -> Prop),
    (forall x : X, P (leaf X x)) -> (forall t1 : tree X, P t1 -> forall t2 : tree X, P t2 -> 
...
Definition P_plus_assoc (n m p : nat) : Prop := n + (m + p) = (n + m) + p.
...
Proof.
  ...
  Case "n = O". reflexivity.
  Case "n = S n'". 
    unfold P_plus_assoc. unfold P_plus_assoc in IHn'. simpl. 
...
Proof.
  ...
  Case "m = S m'". unfold P_plus_comm. unfold P_plus_comm in IHm'. simpl. rewrite <- IHm'.
    rewrite <- plus_n_Sm. reflexivity.  Qed.
